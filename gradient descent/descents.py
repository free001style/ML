...
class BaseDescent:
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE, delta=1.0):
        """
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            less = r[mask] ** 2/2
            more = self.delta * (np.abs(r[~mask]) - self.delta / 2)
            return 1 / x.shape[0] * (less.sum() + more.sum())
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        if self.loss_function is LossFunction.MSE:
            ...
            r = self.predict(x) - y
            mask = (np.abs(r) <= self.delta)
            less =  x[mask].T @ r[mask]
            more = self.delta * x[~mask].T @ np.sign(r[~mask])
            return 1 / x.shape[0] * (less + more)
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        batch_ind = np.random.randint(x.shape[0], size=self.batch_size)
        new_x = x[batch_ind]
        new_y = y[batch_ind]
        return super().calc_gradient(new_x, new_y)
...
class AdaMax(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        self.m = self.beta_1 * self.m + (1-self.beta_1) * gradient
        m_hat = self.m / (1 - self.beta_1 ** self.iteration)
        self.u = np.maximum(self.beta_1 * self.u, np.abs(gradient) + self.eps)
        diff = self.lr() * m_hat / self.u
        self.w -= diff
        return -diff
...
class Nadam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        mu_t = self.beta_1 * (1 - 0.5 * 0.96 ** (self.iteration * self.psi))
        self.mu_prod *= mu_t
        mu_tp1 = self.beta_1 * (1 - 0.5 * 0.96 ** ((self.iteration + 1) * self.psi))

        self.m = self.beta_1 * self.m + (1 - self.beta_1) * gradient
        self.v = self.beta_2 * self.v + (1- self.beta_2) * np.square(gradient)

        m_hat = mu_tp1 * self.m / (1 - self.mu_prod * mu_tp1) + (1 - mu_t) * gradient / (1 - self.mu_prod)
        v_hat = self.v / (1 - self.beta_2 ** self.iteration)
...
class AMSGrad(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff = self.lr() * self.m / (np.sqrt(self.v_hat) + self.eps)
        self.w -= diff
        return -diff
...
class BaseDescentReg(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        # так как у нас последний вес соответствует константному признаку, то его надо занулить
